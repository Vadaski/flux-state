...
```json
{
  "primaryTaskId": "FLUX-001",
  "parallelTracks": ["FLUX-002", "FLUX-003"],
  "dependencyGraph": {
    "FLUX-001": [],
    "FLUX-002": [],
    "FLUX-003": ["FLUX-002"]
...
```bash
node scripts/sprint-board.mjs update --id FLUX-001 --state "In Progress"
node scripts/sprint-board.mjs journal --task FLUX-001 --summary "what changed"
```
...
4. When marking a task `Failed`, record a pitfall entry alongside the state update:
   ```bash
   node scripts/sprint-board.mjs update --id FLUX-001 --state "Failed" \
     --failure-type <gate|acceptance|review> --attempted "..." --hypothesis "..."
   node scripts/sprint-board.mjs pitfall --task FLUX-001 \
     --failure-type <gate|acceptance|review> --attempted "..." --hypothesis "..." \
     [--missing-context "..."]
   ```
